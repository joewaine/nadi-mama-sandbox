...
<template>
  <div
    class="relative bg-stone-900 w-full max-w-[480px] rounded-sm border border-stone-850 shadow-[0_32px_64px_rgba(0,0,0,0.5)] overflow-hidden"
  >
    <!-- Header -->
    <div
      class="px-10 pt-12 pb-8 flex flex-col items-center text-center border-b border-stone-850/30"
    >
    ...
    <div
      class="mx-10 mt-8 px-4 py-3 bg-terracotta-500/10 border border-terracotta-500/30 rounded-sm"
    >
      <p
    ...
    <!-- Form -->
    <form class="p-10 space-y-6" @submit.prevent="submit">
      <div class="space-y-2">
        <label
      ...
      <div class="pt-2">
        <button
          type="submit"
          :disabled="loading"
          class="w-full bg-terracotta-500 hover:bg-terracotta-600 disabled:opacity-50 text-white py-4 rounded-sm text-xs font-semibold tracking-[0.15em] uppercase transition-all duration-300 shadow-lg shadow-terracotta-500/10"
        >
    ...
    <div class="px-10 pb-12 text-center">
    ...
    <!-- Close -->
    <button
      class="absolute top-6 right-6 text-stone-500 hover:text-sand-100 transition-colors"
      aria-label="Close"
      @click="emit('close')"
    >
      <svg
